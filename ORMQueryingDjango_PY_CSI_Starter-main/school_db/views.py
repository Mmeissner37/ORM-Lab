...
def problem_two(request):
    instructors = Instructor.objects.all().order_by('-hire_date')
    
    for instructor in instructors:
      print (f'Full Name: {instructor.first_name} {instructor.last_name}, {instructor.hire_date}')
...
def problem_five(request):
  student = Student.objects.create(first_name = 'Michaela', last_name = 'Meissner', gpa = '3.5')
...
def problem_six(request):
  student_id = 12
  student = Student.objects.filter(pk=student_id).update(gpa='3.8')
    # Make sure to set this equal to the primary key of the row you just created!

  student = Student.objects.get(pk=student_id)
  print(f'ID: {student_id}, Full Name: {student.first_name} {student.last_name}, GPA: {student.gpa}')
...
def problem_seven(request):
  ...
  students = Student.objects.filter(pk=student_id).delete()
